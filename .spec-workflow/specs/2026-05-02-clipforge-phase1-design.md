...
**Tech Stack:** Next.js 14+, React 18, Zustand, @dnd-kit/core, @ffmpeg/ffmpeg@0.12.x, @ffmpeg/util, lucide-react, CSS Modules
...
- Create: `public/fixtures/.gitkeep` (占位，实际视频文件需另行添加)
...
> **Note:** 实际 .mp4 文件需从公开视频源获取并放置在 `public/fixtures/` 目录。Phase 1 验证阶段可使用短时长（<10s）的免费示例视频。

- [ ] **Step 2: 创建 public/fixtures/thumbnails/.gitkeep**

```bash
mkdir -p public/fixtures/thumbnails
touch public/fixtures/thumbnails/.gitkeep
```

- [ ] **Step 3: 创建 public/fixtures/.gitkeep**
...
> **Note:** `import videos from '../../../../fixtures/videos.json'` 需要 tsconfig 的 `resolveJsonModule: true` 和 Next.js 对 JSON 的内置支持。
...
export const useTimelineStore = create<TimelineStore>((set, get) => ({
  ...
  addClip: (materialId) =>
    set((state) => ({
      clips: [
        ...state.clips,
        { materialId, inPoint: 0, outPoint: 0 },
      ],
    })),
  ...
  setClipInOut: (index, inPoint, outPoint) =>
    set((state) => {
      const newClips = state.clips.map((c, i) =>
...
export default function MaterialList() {
  ...
  const handleAdd = async (id: string) => {
    ...
    addClip(id);
    // 设置默认入出点 = 全长
    const lastClips = useTimelineStore.getState().clips;
    const idx = lastClips.length - 1;
    if (idx >= 0) {
...
export default function TimelineTrack() {
  const { clips, playheadTime, selectedClipIndex } = useTimelineStore();
...
export default function VideoPlayer() {
  ...
  // 预览逻辑：播放头移动时 seek 到对应位置
  useEffect(() => {
    if (!videoRef.current || clips.length === 0) return;
    // 根据 playheadTime 计算应该播放哪个 clip 的哪个时间点
    let accTime = 0;
  ...
  const handleTimeUpdate = useCallback(() => {
    ...
    // 更新 playheadTime（简化：暂不基于 video 的 timeupdate 同步）
  }, []);
...
- Modify: `src/app/layout.tsx` (add AppShell import)
...
fixtures/
  videos.json     # 素材元数据
public/fixtures/ # 实际视频文件（需自行添加）
```

## Fixture 视频

Phase 1 使用本地 `public/fixtures/` 目录下的 .mp4 文件。需要自行准备短时长示例视频（推荐使用 https://coverr.co 等免费素材站）。
...
- [x] 技术栈栈匹配（Next.js + React + Zustand + dnd-kit + FFmpeg.wasm）— Task 1
- [x] Zustand Store 设计 — Task 4
- [x] 素材适配器接口 — Task 3
- [x] Fixture videos.json 数据 — Task 2

**No placeholder scan findings.** All steps contain actual code, file paths, and expected outputs.
