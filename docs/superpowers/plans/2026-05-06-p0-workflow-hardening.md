...
Expected: API listens on `127.0.0.1:8011`。

- [x] **Step 5: 启动 Celery worker**
...
如果 3002 被占用，改到空闲端口，并把实际端口写入文档。

- [x] **Step 7: 手动跑一次真实用户路径**

执行顺序：

1. 打开 `http://127.0.0.1:3002/workspace`（或实际端口）
2. 输入一个真实短视频 brief
3. 等待步骤 1-4 返回
4. 确认方案并创建任务
5. 确认页面出现执行交接
...
执行结果（2026-05-06）：
- 本次真实联调复用本机已运行的 `clipforge-postgres` / `clipforge-redis`，backend 使用 `127.0.0.1:8011`，frontend 使用 `127.0.0.1:3002`，queue 为 `clipforge-agent-p0`，provider order 为 `pexels,youtube`。
- 本机没有配置 `PEXELS_API_KEY`，因此不能把依赖 Pexels 的真实素材链路记为已稳定成功。
- 第一条真实联调 session `34ad731d-b7d3-4095-96fa-1ae899481fd1`、job `e27a7261-599d-4062-a6ef-2e1436474b76`。`/workspace -> confirm -> /tasks -> worker` 链路成功，最终失败在 `search_assets`。
- 真实失败原因为：Pexels 没有 key；YouTube 搜索出现 connection reset / timeout。
- 第二条真实联调仍失败在 `search_assets`，但后续 Task 3 修补后 provider failure 聚合更清晰，不再无限累加同类 scene 错误。
...
执行结果（2026-05-06）：
- 已提交 `8e9ded4 fix: dedupe provider failure diagnostics`，对 provider 失败诊断做去重，避免重复错误噪声掩盖真实原因。
- 已提交 `79e41f1 fix: collapse repeated scene provider failures`，折叠重复 scene provider 失败，避免第二次真实联调里同类 scene 错误无限累加。
- 自动验证仍通过：`/Users/linkwind/Code/ClipForge_v2/.venv/bin/python -m unittest tests.test_agent_backend` 报告 68 tests OK。
- 已知 warning 是 SQLAlchemy `datetime.utcnow()` deprecation；本阶段不处理。
...
- [x] **Step 2: 在 `README.md` 明确环境变量与素材源策略**
...
- [x] **Step 4: 在当前 plan 中勾掉已完成项并补阶段总结**
...
阶段总结（2026-05-06）：
- 本地真实工作流已经验证到 `/workspace -> confirm -> /tasks -> worker`，但尚未验证到稳定 MP4 出片。
- 当前失败卡点是 `search_assets`：本机没有 `PEXELS_API_KEY`，YouTube fallback 又受 connection reset / timeout 等外部网络和平台限制影响。
- 这不是任务交接、worker 队列或 render 层失败；失败分类应记为 asset search / provider availability failure。
- 当前还没有达到“可发布演示”标准，因为没有可重复的稳定素材来源。
- 下一小步更适合转去 deterministic asset support / fixture fallback，而不是继续围绕 YouTube 外部不稳定性做恢复闭环。
...
git commit -m "docs: record p0 workflow validation findings"
```
